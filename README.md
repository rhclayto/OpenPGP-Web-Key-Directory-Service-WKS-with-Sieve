...
To register a key with the WKS, send an unencrypted message to the submission-address; wkd-submission@example.com with your public key as an attachment (in .asc format). Dovecot/Sieve will recognize it as a WKS request, and pipe it to the script. The script will use gpg-wks-server to register the key. Notifications and logging (back to Devecot) are issued on success and failure both to the requesting user and to an admin. Adjust the values as needed for your setup. The script is commented to help you understand what is going on. At the bottom is a commented section showing the method I had tried before and never got to work, the method you can find in various places on the Internet. Fixing that was the rationale for this script. I hope it's helpful. Feel free to make improvements.
